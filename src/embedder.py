...
- Utilizes pre-trained transformer models such as BERT, DistilBERT, and others, making it versatile for different NLP tasks.
...
This module requires the transformers library and PyTorch, along with their dependencies, to be installed in your environment.
"""
...
class TextEmbedder:
    ...
    def __call__(self, sentences: list, batch_size: int = -1) -> tuple:
        ...
        padded_ids, padded_idf, seq_lengths, attention_mask, tokens = self.collate_idf(sentences)
    ...
    def tokenize_text(self, input_text: str) -> list:
        ...
        This method tokenizes the given text, prepends the '[CLS]' token at the beginning, appends the '[SEP]' token at the end,
        ...
        tokens = ["[CLS]"] + self.truncate(self.tokenizer.tokenize(input_text)) + ["[SEP]"]
    ...
    def create_idf_dict(self, input_texts: list, n_threads: int = 4) -> defaultdict:
        ...
        Returns:
            defaultdict: A dictionary where keys are token indices (or IDs) and values are their corresponding IDF scores.
                        Tokens not seen in the input texts are assigned a default IDF score based on the total number of documents.
        ...
        idf_dict.update({idx: log((num_docs+1)/(c+1)) for (idx, c) in idf_counter.items()})

        return idf_dict
    
    def padding(self, sequences: list, pad_token: int, dtype: torch.dtype = torch.long) -> tuple:
        """
        ...
        Returns:
            tuple: A tuple containing three elements:
                - padded (torch.Tensor): A tensor of shape (len(sequences), max_seq_length) containing the padded sequences.
                - sequence_lengths (torch.Tensor): A tensor containing the original lengths of each sequence.
                - mask (torch.Tensor): A tensor of the same shape as `padded`, where elements are 1 if part of the original sequence and 0 if padded.
        """
    ...
    def collate_idf(self,
        ...
        Returns:
            tuple: A tuple containing the following elements:
                ...
                - padded_idf (torch.Tensor): A tensor of the same shape as `padded_ids`, containing IDF weights for each token.
                - seq_lengths (torch.Tensor): A tensor containing the original lengths of the sequences before padding.
                - attention_mask (torch.Tensor): A binary tensor indicating which elements are tokens and which are padding.
